...
import { tuCalendarHeatMap }  from "../src/components/tuCalendarHeatMap";
import { Heatmap } from "../src/components/tuCalendarHeatMap/Heatmap";
...
const heatmap = new Heatmap(new Date("2021-08-01"), calendarData, undefined);

describe("tuCalendarHeatMap", () => {
	it("should display the color properly", async () => {
		const wrapper = mount(tuCalendarHeatMap, {
			props: {
				values: calendarData,
				endDate:new Date("2021-08-01")
			}
		});
		const index = getColorIndex(Number(wrapper.findAll(".tu-heatmap-color-2")[56].attributes("data-tippy-content")?.substring(3,11)), heatmap.max);
		expect(wrapper.findAll(".tu-heatmap-color-2")[56].classes()[1]).toBe(`tu-heatmap-color-${index}`);
	});
	it("should display the count properly on that day", async () => {
		const wrapper = mount(tuCalendarHeatMap, {
			props: {
				values: calendarData,
				endDate:new Date("2021-08-01")
		...
		expect(Number(wrapper.findAll(".tu-heatmap-color-2")[56].attributes("data-tippy-content")?.substring(3,11))).toBe(calendarData[56].count);
	});
});