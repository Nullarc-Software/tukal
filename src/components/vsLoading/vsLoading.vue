<template>
	<transition name="vs-loading">
		<div
			v-if="isVisible"
			class="vs-loading"
			:class="[
				`vs-loading--${type || 'default'}`,
				{ 'vs-loading--target': !!target },
				{ 'vs-loading--background': !!background },
			]"
		>
            <div class="vs-loading__load" :style="{
                    transform: `scale(${scale})`
                }">
                <div class="vs-loading__load__animation">
                    <div class="vs-loading__load__percent">
                        {{ percent }}
                    </div>
			...
			<div v-if="progress" class="vs-loading__progress">
				<div class="vs-loading__progress__bar" :style="{
                    width: `${progress}%`
                }">
...
.vs-loading-enter-active, .vs-loading-leave-active {
  transition: opacity 0.5s;
}

.vs-loading-enter, .vs-loading-leave-to {
  opacity: 0;
}
...
.vs-loading--gradient {
  .vs-loading__load__animation {
    ...
    &__1 {
      ...
      animation: rotate 1s linear infinite;
      top: 0px;
      background: linear-gradient(0deg, -getColor("background", 0) 33%, -getColor("color", 1) 100%);
      border-radius: 50%;
    }
