			...
			<template #content>
				<tu-popup-menu>
					...
					<tu-popup-item v-for="(column, index) in table.persistentSettings.columns" :key="index">
						<tu-check-box v-model="column.visibility">
			...
			<table class="tu-table__element" ref="tableElement">
				<thead ref="thead" class="tu-table__thead">
					...
					<tu-th v-if="multiSelect" fixed style="width: 50px">
						<tu-check-box v-model="selectedAll" :indeterminate="table.isPartiallyChecked.value" />
					</tu-th>
				...
				<tbody :id="`${id}-tbody`" :class="{ 'loading-body': !isLoaded }" class="tu-table__tbody">
					...
					<tu-tr v-else v-for="tr in table.dataView.value" :key="tr.index" :data="tr.rowData"
						...
						<tu-td checkbox v-if="multiSelect">
							<tu-check-box v-model="tr.selected" :checked="selectedAll" />
						</tu-td>
						...
						<template #expand v-if="tr.rowData?.['expanded']">
							<component :is="tr.rowData['expanded']?.component ??
...
if (typeof window !== "undefined" && window.VueInstance)
	contextMenuComponent.install(window.VueInstance);
...
const emit = defineEmits<{
	...
	"onRowClicked": [value: unknown];
	"onTableBeginLoad": [value: unknown];
	"onTableEndLoad": [value: unknown];
	"onCellClicked": [value: unknown];
	"onTableConfigUpdated": [value: unknown];
}>();
...
let load: TuLoading | undefined = undefined;

// Add missing variables that are used in template
...
const rowListeners = {
	click: function (event: Event, tr: unknown, row: unknown) {
...
watch(table.loading, (value) => {
	if (value) {
		...
		emit("onTableEndLoad", null);
	}
});

watch(table.dataView, () => {
	emit("update:data", table.dataView.value);
	noOfTableValues.value = table.dataView.value.length;
	isLoaded.value = true;
});

watch(table.getSelectedRows, () => {
	const newVal = table.getSelectedRows.value as Array<unknown>;
	emit("update:modelValue", newVal);
...
onBeforeUnmount(() => {
	// @ts-ignore - Force free up mem on next gc call
	table = undefined;
});

const getNestedField = function (rowData: unknown, key: string) {
...
const startDrag = (evt: DragEvent, header: TuHeaderDefn) => {
	if (isDraggable.value) {
		...
		dragIndex.value = header.index || null;
	}
	else {
		evt.preventDefault();
		evt.stopPropagation();
...
const onDrop = (header: TuHeaderDefn) => {
	if (isDraggable.value) {
		isDrag.value = true;
		dropIndex.value = header.index || null;
		if (props.persistentId && dragIndex.value !== null) {
			table.reOrderTableColumns(
				dragIndex.value,
				dropIndex.value || 0,
				props.persistentId
			);
			const storageItem = localStorage.getItem(`table-${props.persistentId}`);
			if (storageItem) {
				emit(
			...
			table.reOrderTableColumns(dragIndex.value, header.index || 0);
		}
	}
};
</script>
