<template>
	<div class="vs-table-content">
		<header v-if="$slots.header" class="vs-table__header">
			<slot name="header" />
		</header>
		<div class="vs-table"
			:class="{
		...
		>
			<table>
				...
				<tbody ref="tbody" class="vs-table__tbody">
					<slot name="tbody" />
				</tbody>
				<tbody ref="tbody" class="vs-table_not-found">
					<tr>
						<td :colspan="colspan">
		...
		<footer v-if="$slots.footer" class="vs-table__footer">
			<slot name="footer" />
		</footer>
	</div>
	...
	methods: {
		...
		getItemsSearch(min, max) {
			...
			return this.sortItems(this.data)
				.filter((tr) => {
					return (
						this.normalize(this.getValues(tr).toString()).indexOf(
							search
						) != -1
					);
				})
