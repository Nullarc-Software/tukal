<template>
	<div class="tu-hist-container">
		<div :style="centerStyle()" :class="alternative ? 'alter' : 'default'">
			...
			<div class="tu-hist-row" v-for="(ev, index) in  histEvents" :key="index"
				:class="(index % 2 === 0) ? 'tu-hist-row-1' : 'tu-hist-row-2'">
				<section :style="categoryColor(ev)">
					...
					<div v-if="ev.url" style="text-decoration: none;"><a class="tu-hist-title tu-hist-title-router"
							:href="ev.url">{{ ev.title }}</a></div>
